...
export default function CheckoutPage() {
  ...
  // ── State for Razorpay failure fallback ─────────────────────────────────────
    ...
    startTransition(async () => {
      try {
        ...
        if (!result.success || !result.razorpayOrderId) {
          // Razorpay order creation failed — show fallback
          ...
          handler: async (response: {
            ...
            try {
              ...
              if (verification.success) {
                clearCart();
                router.push(`/order-confirmed?ref=${result.dbOrderId}`);
              } else {
                setServerError(
                  "Payment received but verification failed. Contact us at elyarabysweety@gmail.com with your payment ID."
                );
              }
            } catch {
              setServerError(
                "Payment may have been received. Please contact us on WhatsApp to confirm."
              );
            }
          },
          modal: {
            ondismiss: () => {
              // User closed the Razorpay popup without paying — no error needed
            },
          },
        };
        ...
        console.error("[handlePayment] Unexpected error:", e);
        setShowRazorpayFallback(true);
      }
    });
  };
    ...
    <div className="bg-[#faf8f4] min-h-screen">
      ...
      <div className="max-w-6xl mx-auto py-12 px-4 sm:px-6 grid md:grid-cols-2 gap-12">
        ...
        <div className="bg-[#f5f0e8] p-8 h-fit space-y-6">
          ...
          {/* ── Payment Methods ── */}
          <div className="space-y-3">
            {/* Razorpay fallback banner */}
            {showRazorpayFallback && (
              <div className="bg-amber-50 border border-amber-200 p-4 space-y-3 mb-2">
                <div className="flex items-start gap-3">
                  <span className="text-xl flex-shrink-0">⚠️</span>
                  <div>
                    <p className="text-sm font-medium text-amber-800">Online payment is currently unavailable</p>
                    <p className="text-xs text-amber-700/70 mt-1 leading-relaxed">
                      Don&apos;t worry — you can complete your order via UPI on WhatsApp instead. Your order details will be sent directly to our team.
                    </p>
                  </div>
                </div>
                <button
                  onClick={() => { setShowRazorpayFallback(false); handleUpiPayment(); }}
                  className="w-full flex items-center justify-center gap-2 bg-[#25D366] text-white py-3 text-xs tracking-[0.2em] uppercase font-bold hover:bg-[#128C7E] transition-colors"
                >
                  ...
                  Pay via UPI on WhatsApp Instead
                </button>
              </div>
            )}
